...
class task4 {
   ...
   // static int answer = 8;
    static int comp =0;	//to calculate no. of minCostDynamicPro function calls
    ...
    public static int minCostDynamicPro(int i, int j) 
    {
		...
		if(costMap.containsKey(indexToKey(i,j))) //check whether required value is already exists 	
		    return costMap.get(indexToKey(i,j)); //return if exists else calculate
	...
	{
		costMap = null;	//reset hashmap for each calculation
	    comp = 0;  //reset complexity for each calculation 	    
